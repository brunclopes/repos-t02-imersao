...
#Diretorio de origem
gold_path = "/mnt/gold/dim_local/"
 
#Nome da tabela de destino
tb_name = "DW.DIM_LOCAL"

# COMMAND ----------

# DBTITLE 1,Leitura dos arquivos da dim_local - Python
dim_localDF = spark.read.format("delta").load(gold_path)
...
write_db_append(dim_localDF, tb_name)
...
write_db_owerwrite(dim_localDF, tb_name)

# COMMAND ----------

# DBTITLE 1,Consulta os dados da tabela no banco
display(read_db_table('DW.DIM_LOCAL'))

# COMMAND ----------

# DBTITLE 1,Consulta os dados da tabela no banco baseada em uma query
display(read_db_query('SELECT TOP 1 * FROM DW.DIM_LOCAL'))