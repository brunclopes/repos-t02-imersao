...
#Diretorio de origem
gold_path = "/mnt/gold/dim_calendario/"
 
#Nome da tabela de destino
tb_name = "DW.DIM_CALENDARIO"

# COMMAND ----------

# DBTITLE 1,Leitura da tabela dim_artista no datalake - Python
dim_calendarioDF = spark.read.format("parquet").load(gold_path)
...
write_db_append(dim_calendarioDF, tb_name)
...
write_db_owerwrite(dim_calendarioDF, tb_name)

# COMMAND ----------

# DBTITLE 1,Consulta os dados da tabela no banco
display(read_db_table('DW.DIM_CALENDARIO'))

# COMMAND ----------

# DBTITLE 1,Consulta os dados da tabela no banco baseada em uma query
display(read_db_query('SELECT TOP 1 * FROM DW.DIM_CALENDARIO'))